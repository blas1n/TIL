...
bool AlphaMapShader::Initialize(ID3D11Device* device, HWND hWnd, float gammaCorrection)
{
	constexpr static auto MissingShader = TEXT("Missing Shader File");

	constexpr auto vsName = TEXT("Shader/TextureVertexShader.hlsl");
	constexpr auto psName = TEXT("Shader/AlphaMapPixelShader.hlsl");
...
bool AlphaMapShader::Render(ID3D11DeviceContext* context, UINT indexCount, TextureArray* textures,
...
void AlphaMapShader::Release() noexcept
{
	if (matrixBuffer)
	{
...
bool AlphaMapShader::SetParameter(ID3D11DeviceContext* context, TextureArray* textures,
...
void AlphaMapShader::OutputShaderError(ID3D10Blob* errorMsg, HWND hWnd, LPCTSTR shader)
{
